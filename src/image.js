...
export const processImage = async (imageSrc) => {
    ...
    if (stat.isFile()) {
        ...
        const imagePath = removePrefixes(imageSrc, [
            config.data.root,
            config.data.src,
        ...
        if (
            ...
            const { ext } = fileType;
            fancyLog(
                chalk.magenta(`Processing ${ext} image`),
                chalk.cyan(removeRootPrefix(imageSrc)),
                ' to ',
                chalk.cyan(removeRootPrefix(destImagePath)),
            );
            ...
            if (ext === 'svg') {
                fancyLog(
                    chalk.magenta('Processing SVG image'),
                    chalk.cyan(removeRootPrefix(imageSrc)),
                    ' to ',
                    chalk.cyan(removeRootPrefix(destImagePath)),
                );
                ...
                fancyLog(
                    chalk.magenta('Copying unprocessed "image" file'),
                    chalk.cyan(removeRootPrefix(imageSrc)),
                    ' to ',
                    chalk.cyan(removeRootPrefix(destImagePath)),
                );
