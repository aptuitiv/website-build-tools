...
In the [configuration file](/configuration/) you will want to set up how the CSS files will be built. You can also configure [stylelint](https://stylelint.io/) processing.
...
{
    css: {
        buildFiles: ['*.css', 'folder/other-file.css'];
    }
}
```
...
{
    css: {
        buildFiles: ['main.css', 'other-file.css'];
    }
}
```
...
You can override the default configuration in the `stylelint` section of the [configuration file](/configuration/).
