...
<script>products.default.init();</script>;
```
...
// Using the exported function in some other file
<script>products.productsController.init();</script>;
```
...
{
    javascript: {
        entryPoints: ['filename.js', 'path/filename2.js'];
    }
}
```
...
// Using the exported function in some other file
<script>products.helper.init();</script>;
```
        ...
        entryPoints: [
            {
                ...
                bundle: {
                    nodeModules: ['nouislider/dist/nouislider.min.js'],
                },
            },
        ];
    }
